...
async function createSampleWritingGroups(): Promise<
...
async function createSampleWorkshopGroups(
  ...
  for (const instance of instances) {
    if (!instance.workshop || !instance.group) {
      console.error('❌ Invalid workshop or group instance');
      continue;
    }
...
async function addParticipantsToGroups(
  ...
  if (!firstGroup) {
    console.error('❌ No first workshop group available for participant assignment');
    return;
  }
  ...
  if (!secondGroup) {
    console.error('❌ No second workshop group available for participant assignment');
    return;
  }
...
async function createSampleActivities(
  ...
  const firstGroup = workshopGroups[0];
  if (!firstGroup) {
    console.error('❌ No workshop group available for activity creation');
    return;
  }
