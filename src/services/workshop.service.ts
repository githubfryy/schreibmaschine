...
import { isValidSlug, makeUniqueSlug, slugify } from '@/utils/slugify';
...
export class WorkshopService {
  ...
  static async findWithGroups(id: string): Promise<WorkshopWithGroups | null> {
    const workshop = await WorkshopService.findById(id);
    if (!workshop) {
      return null;
    }
  ...
  static async create(data: CreateWorkshop): Promise<Workshop> {
    ...
    const existingSlugs = db
      .prepare('SELECT slug FROM workshops')
      .all()
      .map((w: any) => w.slug);
  ...
  static async update(id: string, data: UpdateWorkshop): Promise<Workshop | null> {
    const existing = await WorkshopService.findById(id);
    if (!existing) {
      return null;
    }
    ...
    if (data.slug !== undefined) {
      ...
      const existingSlugs = db
        .prepare('SELECT slug FROM workshops WHERE id != ?')
        .all(id)
        .map((w: any) => w.slug);
    ...
    return await WorkshopService.findById(id);
  }
  ...
  static async isSlugAvailable(slug: string, excludeId?: string): Promise<boolean> {
    ...
    const result = db.prepare(query).get(...(params as string[])) as { count: number };
