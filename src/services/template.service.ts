...
export class TemplateService {
  ...
  static render(
    ...
    const pageContent = this.renderTemplate(`pages/${pageName}`, data);
    
    if (options.layout !== false && options.layout !== 'none') {
      ...
      return this.renderTemplate(`layouts/${layoutName}`, layoutData);
    }
  ...
  static renderComponent(componentName: string, data: TemplateData = {}): string {
    return this.renderTemplate(`components/${componentName}`, data);
  }
  ...
  private static renderTemplate(templatePath: string, data: TemplateData): string {
    const fullPath = join(this.VIEWS_PATH, `${templatePath}.html`);
    
    // Load template (with caching in production)
    let template: string;
    if (process.env.NODE_ENV === 'production' && this.templateCache.has(fullPath)) {
      template = this.templateCache.get(fullPath)!;
    } else {
      try {
  ...
  private static processTemplate(template: string, data: TemplateData): string {
    ...
    result = result.replace(/\{\{\s*>\s*([^}]+)\s*\}\}/g, (_match, componentName) => {
      try {
        return this.renderComponent(componentName.trim(), data);
      } catch (error) {
