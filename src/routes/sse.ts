  ...
  static removeConnection(
    ...
    for (const conn of connections) {
      if (conn.controller === controller) {
        ...
        console.log(
          `📡 SSE connection removed for ${conn.participantId} in group ${workshopGroupId}`
        );
        break;
      }
    }
    ...
    const event: OnlineStatusEvent = {
      ...
      data: {
        ...
        online_participants: onlineParticipants.map((p) => ({
          ...
          role: 'participant' as const, // Default role, should get from DB if needed
          last_seen: p.lastSeen,
          device_info: `${p.deviceCount} device(s)`,
        })),
        total_participants: onlineParticipants.length, // TODO: Get actual total from DB
        online_count: onlineParticipants.length,
      },
...
export const sseRoutes = new Elysia({ name: 'sse' })
  ...
  .get(
    ...
    async ({
      ...
      workshopGroup?: import('../types/database').WorkshopGroup;
