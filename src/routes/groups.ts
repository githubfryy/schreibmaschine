...
export const groupRoutes = new Elysia()
  ...
  .get('/resolve-url', async ({ query, set }) => {
    try {
      const { path } = query;
  ...
  .get('/gruppe-:shortId', async ({ params, set, headers }) => {
    try {
      const resolved = await UrlService.resolveByShortId(params.shortId);
      
      if (!resolved) {
        set.status = 404;
        const html = TemplateService.render('error', {
          error_icon: '🔍',
  ...
  .get('/:workshopSlug/:groupSlug/vorraum', async ({ params, set }) => {
    try {
  ...
  .get('/:workshopSlug/:groupSlug', async ({ params, set, headers }) => {
    try {
      const resolved = await UrlService.resolveBySemanticUrl(params.workshopSlug, params.groupSlug);
