...
import { participantMiddleware } from '../../middleware/participant';
import { z } from 'zod';
...
const CollaborativeDocumentSchema = z.object({
  documentId: z.string(),
  operations: z.array(z.any()), // Loro operations will be validated separately
  baseVersion: z.number()
});

export const documentsRoutes = new Elysia({ prefix: '/documents' })
  .use(participantMiddleware)
  .decorate('documentService', new DocumentService())

  // Save individual writing
  .post('/individual', async ({ body, documentService, participant }) => {
    if (!participant?.id) {
  ...
  .get('/individual/:groupId', async ({ params, documentService, participant }) => {
    if (!participant?.id) {
      throw new Error('Authentication required');
    }

    const document = await documentService.getIndividualDocument(
      participant.id,
      params.groupId
    );
  ...
  .get('/collaborative/:documentId', async ({ params, documentService, participant }) => {
    if (!participant?.id) {
      throw new Error('Authentication required');
    }
  ...
  .post('/collaborative/:documentId/operations', async ({ 
    params, 
    body, 
    documentService, 
    participant 
  ...
  .get('/export/:groupId/:format', async ({ 
    params, 
    documentService, 
    participant,
    set 
    ...
    const exportData = await documentService.exportGroupDocuments(
      params.groupId,
      format
    );
  ...
  .get('/activity/:activityId', async ({ params, documentService, participant }) => {
    if (!participant?.id) {
      throw new Error('Authentication required');
    }
  ...
  .delete('/:documentId', async ({ params, documentService, participant }) => {
    if (!participant?.id) {
      throw new Error('Authentication required');
    }

    const hasAccess = await documentService.canDeleteDocument(
      params.documentId,
      participant.id
    );
    if (!hasAccess) {
